/*
 * Calls `contribute(amount)` on an already-deployed Milestone contract.
 * The amount is a private circuit argument — it never touches the ledger
 * unless it pushes the hidden running total past the next milestone.
 *
 * Usage:
 *   CONTRACT_ADDRESS=<address> \
 *   WALLET_SEED=<hex> OWNER_SECRET_KEY=<hex> \
 *   AMOUNT=40 npm run contribute:preview
 */
